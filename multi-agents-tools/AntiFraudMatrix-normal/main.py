...
def main():
    # 解析命令行参数
    parser = argparse.ArgumentParser(description="多智能体日常对话生成系统")
    parser.add_argument("--fraud_type", default="咨询客服", help="对话场景类型")
    parser.add_argument("--user_age", type=int, default=45, help="用户年龄")
    parser.add_argument("--awareness", default="中", help="用户沟通风格: 低, 中, 高")
    ...
    left_agent = LeftAgent(
        model=args.model,
        fraud_type=args.fraud_type,  # 保持原变量名fraud_type
    ...
    right_agent = RightAgent(
        ...
        user_profile={
            "age": args.user_age,
            "awareness": args.awareness,  # 保持原变量名awareness
            "occupation": "教师"  # 可以添加更多参数
        },
        base_url=args.base_url
    )
